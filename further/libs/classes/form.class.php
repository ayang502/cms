...
class form {
	 ...
	 * @param $str 属性
	 */
	public static function select_template_further($style, $dir = '', $display = '') {
        $tpl_root    = pc_base::load_config('system', 'tpl_root');
        if (empty($dir)) {
		    $templatedir = PC_PATH. "{$tpl_root}{$style}{$dir}/";
        } else {
            $templatedir = $dir;
        }
        $files       = glob("{$templatedir}*");
        natcasesort($files);
        $html .= "<ul class=\"jqueryFileTree\" display=\"{$display}\">";
        foreach ($files as $k=>$file) {
            if (is_dir($file)) {
                $basename = basename($file);
                $html .= "<li class=\"directory collapsed\"><a href=\"#\" rel=\"" . htmlentities($file) . "/\">" . htmlentities($basename) . "</a></li>";
                unset($files[$k]);
            }
        }
        foreach ($files as $k=>$file) {
            if ($file) {
                ...
                $html .= "<li class=\"file ext_$ext\"><a href=\"#\" rel=\"" . htmlentities($file) . "\">" . htmlentities($basename) . "</a></li>";
            }
        }
        $html .= "</ul>";
        return $html;
