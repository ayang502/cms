...
class category extends admin {
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			if($type==0) {
				...
				include $this->admin_tpl('category_add_further');
			} elseif ($type==1) {
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$type = $_GET['type'];
			if($type==0) {
				include $this->admin_tpl('category_edit_further');
			} elseif ($type==1) {
	...
	*/
     ...
     * by laoyang
	 */
	public function public_tpl_file_list_further() {
		$style = isset($_GET['style']) && trim($_GET['style']) ? trim($_GET['style']) : exit(0);
		$catid = isset($_GET['catid']) && intval($_GET['catid']) ? intval($_GET['catid']) : 0;
		$batch_str = isset($_GET['batch_str']) ? '['.$catid.']' : '';
		if ($catid) {
		...
		pc_base::load_sys_class('form','',0);
        $html = array(
            'category_template' => form::select_template_further($style, ""), 
            'list_template' => form::select_template_further($style, ""),
            'show_template' => form::select_template_further($style, "")
        );
		if (CHARSET == 'gbk') {
			$html = array_iconv($html, 'gbk', 'utf-8');
		}
     ...
     * by laoyang
	 */
	public function public_tpl_file_list_further_tree() {
		...
		pc_base::load_sys_class('form','',0);
        $template = form::select_template_further($style, $dir, "none"); 
        echo $template;
	}
}
?>
