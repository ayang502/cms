...
<link href="<?php echo JS_PATH?>filetree/jqueryFileTree.css" rel="stylesheet" type="text/css" media="screen" />
<script type="text/javascript" src="<?php echo JS_PATH?>filetree/jquery.easing.js"></script>
<script type="text/javascript" src="<?php echo JS_PATH?>filetree/jqueryFileTree.js"></script>
...
<tr>
 ...
 <td>
        <div> 当前选择: <input id="category_cur" type="text" name="setting[category_template]" readonly="true" value="<?php echo $setting['category_template'];?>" style="width:500px"/></div>
        <p></p>
        <div id="category_template" class="demo"></div>
        </td>      
</tr>
	  <tr>
        <th width="200"><?php echo L('category_list_tpl')?>：</th>
<td>
        <div> 当前选择: <input id="list_cur" type="text" name="setting[list_template]" readonly="true" value="<?php echo $setting['list_template'];?>" style="width:500px"/></div>
        <div id="list_template" class="demo"></div>
		</td>
      </tr>
	  <tr>
        <th width="200"><?php echo L('content_tpl')?>：</th>
<td>
        <div> 当前选择: <input id="show_cur" type="text" name="setting[show_template]" readonly="true" value="<?php echo $setting['show_template'];?>" style="width:500px"/></div>
        <div id="show_template" class="demo"></div>
		</td>
      </tr>
...
<!--
    ...
    var style = '<?php echo $setting['template_list']?>'; 
    inter_script = '';
    if (style != "" ){
        inter_script = '?m=admin&c=category&a=public_tpl_file_list_further_tree&style='+style+'&catid='+catid;
    }
    var root_dir = '<?php echo PC_PATH . pc_base::load_config('system', 'tpl_root');?>';
	function load_file_list(id) {
        if (id == '') {
            return false;
        }
        var inter_script = '?m=admin&c=category&a=public_tpl_file_list_further_tree&style='+id+'&catid='+catid;
        $('#category_template').fileTree({ root: '', script: inter_script, folderEvent: 'dblclick', expandSpeed: 1, collapseSpeed: 1 }, function(file) {
            $('#category_cur').val(file.replace(root_dir, '/'));
        });

        $('#list_template').fileTree({ root: '', script: inter_script, folderEvent: 'dblclick', expandSpeed: 1, collapseSpeed: 1 }, function(file) {
            $('#list_cur').val(file.replace(root_dir, '/'));
        });

        $('#show_template').fileTree({ root: '', script: inter_script, folderEvent: 'dblclick', expandSpeed: 1, collapseSpeed: 1 }, function(file) {
            $('#show_cur').val(file.replace(root_dir, '/'));
        });
    }
    if (style != '') {
        $('#category_template').fileTree({ root: '', script: inter_script, folderEvent: 'dblclick', expandSpeed: 1, collapseSpeed: 1 }, function(file) {
