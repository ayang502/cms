...
class content_tag {
     ...
     * 自定义相关文章
     */
    public function p_relation($data) {
        ...
        foreach ($tmp as $v) {
            ...
            //开始获取内容
            $this->db->set_model($modelid);
            $tablename = $this->db->table_name = $this->db->db_tablepre.$this->db->model_tablename;
            $where = "id = {$id} and status = 99";
            $list1 = $this->db->get_one($where, '*', $limit);

		    $this->db->table_name = $tablename . '_data';
            $list2 = $this->db->get_one("id={$id}", '*', $limit);
            $content[] = $list1 + $list2;
        }
        return $content;
    }
