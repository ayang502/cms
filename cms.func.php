...
function CMS_SEARCH($params)
{
	...
	*/
	...
	//如果是分页调用模式.continue...
	if($PageMode) {
		$sql_num = "SELECT Count(*) as TotalNum  FROM $table->content_index i,$table_name c where (i.PublishDate <= UNIX_TIMESTAMP() ) AND i.Type=1 AND i.IndexID =c.IndexID  AND i.State!=-1  $list_where $where ";
		//echo $sql_num;exit;
		$result = $db->getRow($sql_num, 2);
		$TotalNum = $result[TotalNum];
		$TotalPage = ceil($result[TotalNum]/$offset);
	...
	$sql_query  = "SELECT i.NodeID,i.ContentID,i.State,i.URL,i.IndexID,i.PublishDate,i.Type,i.Sort,i.Pink, $c_return  FROM $table->content_index i ,$table_name c where (UNIX_TIMESTAMP() >= i.PublishDate)  AND i.IndexID =c.IndexID  AND i.State!=-1 AND i.Type=1 $list_where $where  $list_orderby $list_limit";
	//echo $sql_query; 
	$result = $db->Execute($sql_query, $cache);
	while(!$result->EOF) {
		$NInfo = $iWPC->loadNodeInfo($result->fields[NodeID]);
